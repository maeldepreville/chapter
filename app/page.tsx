...
function renderReadingBookmark(progress: OptionalProgress, compact = false) {
  const fullLabel = `Marque-page · p. ${progress.page}${progress.totalPages ? ` sur ${progress.totalPages}` : ""}`;

  return (
    <span className={`reading-bookmark${compact ? " reading-bookmark--compact" : ""}`} aria-label={fullLabel}>
      ...
      <span className="reading-bookmark-copy">
        {compact ? (progress.totalPages ? `/ ${progress.totalPages}` : "Marque-page") : fullLabel}
      </span>
    </span>
  );
}
