...
type ReaderAvatarIdentity = { name: string; initials: string; avatarSrc?: string };

function ReaderAvatar({ actor, sizes = "40px" }: { actor: ReaderAvatarIdentity; sizes?: string }) {
  return <span className="avatar" aria-hidden="true">{actor.avatarSrc ? <Image src={actor.avatarSrc} alt="" fill sizes={sizes} unoptimized /> : actor.initials}</span>;
}
...
export function ProfileView({ owner, works, following, onToggleFollow, onOpenWork, onOpenHonors, onOpenList, photo, onEditPhoto, onRemovePhoto, equippedTitle, showcase, personalReviews = [], displayName, blocked = false, onToggleBlock, onBack, backLabel = "Retour", favoriteWorkIds, personalLists, curationWorks, onSaveFavorites, onSaveLists }: ProfileProps) {
  ...
  const editableWorks = curationWorks ?? works;
  const editableWorkIds = new Set(editableWorks.map((work) => work.id));
  const editableFavorites = (favoriteWorkIds ?? profile.favorites).filter((id) => editableWorkIds.has(id));
  const editableLists = profileLists.map((list) => ({ ...list, workIds: list.workIds.filter((id) => editableWorkIds.has(id)) }));
  ...
  return (
    <section className={`profile-page ${isOwnProfile ? "profile-home" : "profile-public"}`} aria-labelledby="profile-name">
            ...
            <div className="profile-card-flip">
              <div className="profile-identity-card profile-card-front" aria-hidden={cardFlipped} inert={cardFlipped ? true : undefined}>
                ...
                <div className="profile-heading-copy">
                  ...
                  {!isOwnProfile && <div className="profile-actions">{!blocked && <button className="primary-action profile-follow-action" type="button" aria-pressed={following} onClick={onToggleFollow}>{following ? "Suivi" : "Suivre"}</button>}{onToggleBlock && <button className="quiet-action" type="button" onClick={() => blocked ? onToggleBlock() : setBlockConfirm(true)}>{blocked ? "Débloquer" : "Bloquer"}</button>}</div>}
                </div>
      ...
      <Fade show={isOwnProfile && curationMode === "favorites"} kind="modal">{isOwnProfile && curationMode === "favorites" && onSaveFavorites && <FavoritesEditor works={editableWorks} selected={editableFavorites} onSave={onSaveFavorites} onClose={() => setCurationMode(null)} />}</Fade>
      <Fade show={isOwnProfile && curationMode === "lists"} kind="modal">{isOwnProfile && curationMode === "lists" && onSaveLists && <ListsEditor works={editableWorks} lists={editableLists} onSave={onSaveLists} onClose={() => setCurationMode(null)} />}</Fade>
      <Fade show={blockConfirm} kind="modal">{blockConfirm && (
        <Modal className="profile-photo-remove-overlay" alert labelledBy="block-account-title" initialFocus="[data-cancel-block]" onRequestClose={() => setBlockConfirm(false)}>
          ...
          <section className="profile-photo-remove-dialog">
            <p className="eyebrow">Bloquer ce compte</p>
            <h2 id="block-account-title">Masquer {profile.firstName} partout ?</h2>
            <p>Ses critiques, réponses et listes seront masquées, et les interactions directes seront interrompues.</p>
            <div className="modal-actions"><button data-cancel-block className="quiet-action" type="button" onClick={() => setBlockConfirm(false)}>Annuler</button><button className="destructive-action" type="button" onClick={() => { onToggleBlock?.(); setBlockConfirm(false); }}>Bloquer</button></div>
          </section>
        </Modal>
      )}</Fade>
    </section>
...
export function PublicListView({ owner, listId, list: suppliedList, works, following, blocked = false, displayName, photoSrc, onToggleFollow, onOpenProfile, onOpenWork, onBack, backLabel }: { owner: ProfileOwner; listId: string; list?: EditableProfileList; works: readonly SocialWork[]; following: boolean; blocked?: boolean; displayName?: string; photoSrc?: string; onToggleFollow: () => void; onOpenProfile: () => void; onOpenWork: (id: string) => void; onBack: () => void; backLabel: string }) {
  ...
  const author = { ...actor, name: canonicalName, initials: isOwnList ? readerInitials(canonicalName) : actor.initials, avatarSrc: isOwnList ? photoSrc : undefined };
  ...
  return (
    <section className="destination-page public-list-page" aria-labelledby="list-page-title">
      ...
      <header className="public-list-heading">
        ...
        <div className="list-author-row"><button className="identity-link" type="button" onClick={onOpenProfile}><ReaderAvatar actor={author} /><span><strong>{author.name}</strong><small>{blocked ? "Compte bloqué" : authorRelationship}</small></span></button>{!isOwnList && !blocked && <button className="primary-action profile-follow-action" type="button" aria-pressed={following} onClick={onToggleFollow}>{following ? "Suivi" : "Suivre"}</button>}</div>
      </header>
