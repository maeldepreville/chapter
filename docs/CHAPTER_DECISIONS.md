...
- Phase 11 — états transversaux, cohérence, responsive et accessibilité : **premier ensemble accepté, publication différée au jalon regroupé demandé par l'utilisateur ; deuxième ensemble proposé, non autorisé**. La recette navigateur reste à effectuer. Le plan initial s'arrête à cette consolidation ; l'authentification, l'onboarding et les autres fonctions exclues nécessiteront un périmètre ultérieur explicitement validé.
...
Le deuxième ensemble est proposé, sans autorisation de développement : traitement du catalogue vide/incomplet sans remplacer une œuvre par une autre, états vides du Journal déjà validés, repli éditorial EH1 sans historique et couverture typographique après échec d'image. Trois erreurs de rendu sur catalogue vide et une liste répétant six fois une œuvre unique ont été reproduites avec des données de test. Les constats détaillés P11-F05 à F09 figurent dans le document de phase ; aucun nouveau correctif applicatif n'est implémenté dans ce tour.
