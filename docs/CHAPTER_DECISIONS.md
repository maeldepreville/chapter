...
**Suite actuelle : volet 4 constitué puis repris après recette, P11-F34 publié en version Sites 30.** Le 27 août 2026, « Je valides avec AM1. Continuons. » valide le **fondu seul**, sans déplacement des surfaces. Le 1er septembre, l'utilisateur autorise l'implémentation immédiate des correctifs diagnostiqués et révise la méthode future. **P11-F18 à F21 sont traités dans le dépôt candidat :** verso isolé, faces étanches, coordonnées tactiles adaptées et continuité pincement → glissement. **P11-F22 à F27** traitent le reflow et les contrôles accessibles. **P11-F28 à F30** renforcent le verso mobile, harmonisent « Suivre/Suivi » et remplacent la croix native par « Effacer ». **P11-F31 et F32** règlent la durée et l'accessibilité de la fiche d'honneur. **P11-F33** conserve le propriétaire d'une liste entre le profil et sa destination. **P11-F34** transmet la liste réellement choisie depuis Découvrir, remplace les comparaisons de noms par des identifiants stables, centralise acteurs, listes et URL, puis consigne les fixtures qui devront être remplacées par le backend. Le volet 4 vérifie aussi les liaisons Découvrir/profil/liste/honneurs, statut/date/Journal et ajout/annulation. PFP1, QR1, QRM1b, QRP1b, HDE1, HMT1, les assets et CT1 restent inchangés. **Build de production, lint, `git diff --check` et 98/98 tests réussis.** La version Sites 30 est la base visible de recette. Cette publication ne valide pas le jalon et ne synchronise pas GitHub. La recette à 200 % de texte, Safari mobile, desktop étroit, clavier et technologies d'assistance reste indispensable ; aucune conformité WCAG globale n'est revendiquée. Le détail des données intentionnellement statiques est dans `PROTOTYPE_DATA_REGISTER.md` et le bilan candidat dans `PHASE_11_BILAN_ET_CLOTURE.md`.

**Reprise sociale P11-F35 :** la recette de la version 30 invalide les traces de profil écrites séparément des critiques d'œuvre et la limitation des profils navigables à Lina. Le contrat devient transversal : toute trace publique provient de la même critique que la page d'œuvre et toute identité sociale affichée — avatar comme nom, critique comme réponse — ouvre son profil. Maël, Lina, Théo et Inès disposent désormais d'une destination et d'un suivi indépendants dans le candidat ; le backend social et les routes publiques dynamiques restent hors lot 1.
