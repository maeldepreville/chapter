...
- Un correctif de recette remplace le composant QR exécuté dans le navigateur, incompatible avec la chaîne de production actuelle et responsable d'une page blanche à l'ouverture de « Mon profil », par un SVG QR statique généré localement. L'URL encodée, la composition du verso et toutes les interactions QR1 restent inchangées ; le profil ne dépend plus d'un moteur QR au moment de son affichage.
