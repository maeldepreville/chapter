...
| Découvrir | `DiscoverView` dans `app/phase10.tsx` | `catalogue.ts`, `cover-frame.tsx` | `social-controls`, `phase11-polish`, `phase11-journeys` |
| Profil et carte de lecteur | `ProfileView` dans `app/phase10.tsx` | `profile-share.ts`, `photo-processing.ts` | `profile-layout`, `profile-qr`, `phase11-recovery`, `phase11-polish` |
...
| Listes publiques et conversations | `PublicListView` / `SocialReviews` dans `app/phase10.tsx` | `catalogue.ts` | `social-controls`, `phase11-recovery`, `phase11-journeys` |
