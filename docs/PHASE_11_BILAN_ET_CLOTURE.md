...
La version Sites **28** du présent candidat est désormais déployée en accès propriétaire à la demande explicite de l'utilisateur, afin de permettre sa recette sur `https://chapter-reading.smrdsh.chatgpt.site`. Elle remplace la version 27 et rend les actions de la fiche d'honneur accessibles au pointeur sans réintroduire une persistance à l'échelle de toute la collection. Cette publication ne vaut pas validation. Après validation explicite du jalon regroupé, la synchronisation GitHub prévue par `AGENTS.md` devra encore être réalisée et vérifiée.
