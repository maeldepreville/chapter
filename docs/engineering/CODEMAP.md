...
| Profil, carte de lecteur et curation | `ProfileView` dans `app/phase10.tsx` | `profile-curation.tsx`, `prototype-data.ts`, `site-config.ts`, `profile-share.ts`, `photo-processing.ts` | `profile-curation`, `profile-layout`, `profile-qr`, `hardcoded-data`, `phase11-recovery`, `phase11-polish` |
...
| Fondation candidate de la page œuvre | `PublicWork` dans `app/p1-public.tsx`, vue œuvre dans `app/page.tsx` | `book-work-cover.tsx`, `work-past-notes.tsx`, `app/recette/oeuvre/work-recipe-lab.tsx`, `p1-public.css`, `globals.css` | `work-foundation`, `p4-social`, `phase11-accessibility`, `phase11-empty-states` |
