...
Statut : **jalon validé par l’utilisateur et synchronisé sur la branche GitHub `refonte-pre-lot-2`**.
...
- fusion vers `main` avant la recette complète P7.
...
## Corrections de recette clôturées le 6 septembre 2026
...
- **Responsive mobile de Recherche et de l’inscription :** après la première correction, le croquis conservait une largeur fixe de 38 rem et la feuille d’inscription compensait le retrait du dialogue avec une largeur supérieure à 100 %. Le croquis dépend désormais du viewport avec une limite maximale ; la feuille basse occupe exactement la largeur disponible, respecte les zones sûres et utilise la hauteur dynamique du mobile.
- **Contrôle natif de Recherche :** le bouton d’effacement injecté par le navigateur est masqué sur les moteurs concernés afin que l’action Chapter `Effacer` reste l’unique contrôle visible.

## Validation finale

L’utilisateur valide explicitement P2 le 6 septembre 2026 après recette de la version Sites 38. Le parcours du premier repère, la reprise éditoriale de Recherche, le sélecteur AJ2 mobile, le responsive du croquis et de l’inscription ainsi que le contrôle d’effacement sont acceptés. Les 118 tests, le lint, la construction de production et `git diff --check` réussissent. Le jalon complet est synchronisé sur `refonte-pre-lot-2` sans fusion vers `main` ; P3 — Usage personnel quotidien devient l’unique prochaine tranche.
