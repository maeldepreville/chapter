...
- P4 — Dimension sociale possède une candidate corrigée le 6 septembre 2026. La page d’œuvre publique porte désormais les critiques et conversations ; auteurs, profils, listes et résultats de Recherche forment un parcours public continu. Publier, répondre ou suivre déclenche au premier geste une identité minimale, puis restitue l’action sans publication implicite. Journal, Bibliothèque, Découvrir et Recherche restent stables dès la visite publique : les deux espaces personnels montrent d’abord un seuil d’action fortement identifiable autour d’une œuvre réelle, puis reçoivent les données privées après un premier enregistrement sans mutation de la barre. Les informations explicatives restent présentes dans un second rythme sous l’action principale. Le mode d’accès, l’identité publique et la maturité personnelle demeurent indépendants. Une pile contextuelle commune aux œuvres, profils, listes et honneurs restitue au retour la source exacte, son URL, son entité et son défilement. Cette candidate reste en recette : elle ne vaut ni validation ni synchronisation GitHub. Le détail actif est dans [`P4_DIMENSION_SOCIALE.md`](../milestones/P4_DIMENSION_SOCIALE.md).
