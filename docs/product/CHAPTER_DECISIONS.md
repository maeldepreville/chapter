...
- Consigne prioritaire du 1er septembre 2026 : une demande de correctif ou d'implémentation autorise sa réalisation directe afin que l'utilisateur puisse l'évaluer en contexte. Le cycle devient demande → analyse ciblée → implémentation candidate → évaluation utilisateur → validation ou reprise.
- Un arbitrage préalable reste nécessaire lorsqu'une ambiguïté produit ou visuelle modifierait matériellement le résultat. Publication, synchronisation GitHub, déploiement et action destructive conservent leurs autorisations propres ; une implémentation candidate ne les autorise pas implicitement.
...
- P1 — Premier contact public adopte une racine `Découvrir`, une recherche immédiate par œuvre ou auteur et une fiche œuvre publique sans commandes personnelles. Le jalon et ses reports vers P2, P4 et P6 sont détaillés dans [`P1_PREMIER_CONTACT_PUBLIC.md`](../milestones/P1_PREMIER_CONTACT_PUBLIC.md). P1 est validé sur la version Sites 34 et synchronisé sur la branche GitHub de refonte, assets sources compris après autorisation explicite ; P2 devient le prochain jalon, sans fusion vers `main`.
- Retour de recette P1 sur la version Sites 32 : les retours, la Recherche et sa composition sont jugés corrects et restent inchangés. L'ouverture est reprise car son titre monumental rompt le rythme et parce que l'univers visuel dépend trop exclusivement des couvertures. Le correctif réduit et simplifie le titre, resserre l'œuvre d'entrée et introduit une illustration éditoriale originale issue des familles mémoire et découverte ; il reste candidat jusqu'à nouvelle recette.
...
- P3 — Usage personnel quotidien est validé le 6 septembre 2026 sur la version Sites 42. Le Journal, la Bibliothèque privée de 500 œuvres chargée volontairement par lots, les notes privées et la relecture sans duplication partagent le même état de session. Le marque-page final est un asset matériel latéral qui n’affiche que la page actuelle ; la dernière trace adopte un feuillet compact à contour unique. La Bibliothèque remplie s’appuie sur les couvertures plutôt que sur une illustration supplémentaire. Le jalon peut être synchronisé sur `refonte-pre-lot-2` sans fusion vers `main` ; P4 devient la prochaine tranche.
- P4 — Dimension sociale est validé le 6 septembre 2026 sur la version Sites 49. La page d’œuvre publique porte désormais les critiques et conversations ; auteurs, profils, listes et résultats de Recherche forment un parcours public continu. Publier, répondre ou suivre déclenche au premier geste une identité minimale, puis restitue l’action sans publication implicite. Journal, Bibliothèque, Découvrir et Recherche restent stables dès la visite publique : les deux espaces personnels montrent d’abord un seuil d’action fortement identifiable porté par un asset éditorial sans œuvre reconnaissable, puis reçoivent les données privées après un premier enregistrement sans mutation de la barre. L’action principale ouvre Découvrir, qui convient à une intention encore ouverte ; Recherche demeure l’alternative explicite. Les informations explicatives restent présentes dans un second rythme sous l’action principale. Le mode d’accès, l’identité publique et la maturité personnelle demeurent indépendants. Une pile contextuelle commune aux œuvres, profils, listes et honneurs restitue au retour la source exacte, son URL, son entité et son défilement. P4 est synchronisé sur `refonte-pre-lot-2` sans fusion vers `main` ; P5 devient la prochaine tranche. Le détail final est dans [`P4_DIMENSION_SOCIALE.md`](../milestones/P4_DIMENSION_SOCIALE.md).
- Ajustement P5 du 8 septembre 2026 : l'import `chapter-export` reste une fusion privée. Les lectures reconnues complètent l'état de session et les traces importées n'effacent pas l'historique local absent de l'archive. Une critique importée est conservée comme brouillon privé, avec une trace privée identifiable, mais ne rejoint ni les critiques publiques ni le Profil avant une republication explicite. Une critique déjà publique dans la session demeure inchangée. Cette séparation remplace l'ancien couplage implicite entre présence d'un texte de critique et visibilité publique.
- P5 — Confiance et contrôle est validé explicitement le 8 septembre 2026 sur la version Sites 66. L'identité publique se propage partout, la photo personnalise la coque, le Profil et les listes secondaires suivent la composition superposée acceptée, les scrollbars et mouvements sont harmonisés, le blocage agit transversalement et l'archive se réimporte uniquement dans l'espace privé. L'audit final ferme le menu du compte à l'entrée du Profil et empêche son auto-empilement. P5 peut être synchronisé sur `refonte-pre-lot-2` sans fusion vers `main` ; P6 devient la prochaine tranche.
- Ajustement de recette P7 du 10 septembre 2026 : N2C devient une pile globale limitée à quatre retours, ordonnée de l'ancien en haut au plus récent en bas, avec réagencement fluide, pause au survol/focus et annulation propre à chaque action. Les réussites d'import et d'export quittent ce retour bref pour une carte éditoriale dédiée : fermeture par croix ou extérieur, contrôle explicite de confidentialité, puis accès à la Bibliothèque après import ou au Journal après export.
- Ajustement de recette P7 du 11 septembre 2026 : sur mobile, le sommaire de Réglages et données reste collant sous la barre de fermeture et ses ancres compensent les deux bandeaux. Le Profil propriétaire expose désormais « Modifier » pour choisir jusqu’à trois œuvres de chevet et « Gérer mes listes » pour créer, modifier ou supprimer ses rayonnages publics. Ces actions restent absentes des portraits tiers ; leurs effets sont locaux à la session tant que le lot 2 n’apporte pas la persistance.
- Ajustement de recette P7 du 11 septembre 2026 : les sélecteurs des œuvres de chevet et des listes utilisent la Bibliothèque privée comme unique source de choix, sans dépendre de ses filtres visibles. Sur les fiches œuvre, la critique et les réponses du lecteur courant portent le libellé contextuel « Vous » ; son nom public reste réservé au profil et aux vues d’autrui.
- Ajustement de recette P7 du 11 septembre 2026 : chaque notification temporaire possède son propre compte à rebours de cinq secondes, conservé lorsqu’elle est mise en pause et indépendant des nouvelles notifications. Sa disparition reprend exactement l’animation d’apparition en sens inverse avant le retrait de la pile.
- Ajustement de recette P7 du 11 septembre 2026 : lorsqu’aucun badge n’est mis en avant, la section « Chapitres d’honneur » du Profil conserve une présence éditoriale illustrée. Le propriétaire peut rejoindre directement la sélection de ses acquis ; le portrait public reçoit un texte neutre sans action de gestion.
- Ajustement de recette P7 du 11 septembre 2026 : l’ex-libris vide appartient exclusivement aux Chapitres d’honneur. Les Œuvres de chevet vides reçoivent une illustration propre — trois livres sans titre près d’une lampe — afin de distinguer la curation intime des distinctions.
- Ajustement de recette P7 du 11 septembre 2026 : toute première action personnelle ou publique depuis la visite sans compte passe par une même carte de création d’espace. Elle réunit « Nom de lecteur », adresse e-mail et mot de passe ; le nom de lecteur devient la signature publique destinée à être unique, avec contrôle de disponibilité reporté au backend. Une critique reprend ensuite exactement là où elle avait été initiée.
- Ajustement de recette P7 du 11 septembre 2026 : l’identité courante possède une seule projection visuelle. Toute icône qui la représente — navigation, Profil, critiques, réponses et listes — affiche la photo recadrée lorsqu’elle existe, sinon les initiales recalculées depuis le Nom de lecteur.
- Ajustement de recette P7 du 11 septembre 2026 : une note privée existante et une critique publique disposent chacune d’une suppression explicite depuis leur éditeur, protégée par une confirmation intégrée et suivie d’un retour annulable. Retirer une critique efface sa trace publique mais conserve l’évaluation privée de l’œuvre.
- Clôture P7 du 12 septembre 2026 : après l’audit navigateur final, l’utilisateur valide explicitement la refonte visible pré-lot 2. La version Sites 84 aligne définitivement Réglages et création de compte sur « Nom de lecteur », destiné à être unique sur Chapter et limité à 40 caractères. Les six parcours P7, les cinq états P6, les notifications indépendantes, les suppressions réversibles, les états vides, la curation depuis la Bibliothèque, « Vous » et la projection unique de l’avatar sont couverts par 177/177 tests, le lint, la construction de production, `git diff --check` et l’inspection navigateur. La fusion vers le `main` GitHub public est préparée mais attend une autorisation explicite distincte ; le lot 2 backend reste un cadrage séparé.
- P6 — États transversaux est validé explicitement le 10 septembre 2026 sur la version Sites 72. Les routes publiques inconnues n'affichent plus une entité de remplacement ; Chapter possède des frontières globales de chargement, erreur récupérable et page absente. Le clavier dispose d'un lien d'évitement et d'une cible principale focalisable, Recherche expose son état courant et le mouvement réduit neutralise aussi le squelette. L'atelier isolé `/recette/p6` rend observables les états normal, vide, chargement, erreur et contenus extrêmes sans rejoindre la navigation produit. Les corrections de recette suppriment le flash des petites surfaces, généralisent la hiérarchie et le contraste des couvertures, bornent les traces du Profil, harmonisent les états vides illustrés et préservent les grandes cartes comme cartes sur mobile. Le commit `a5ce28e` clôt P6 et est synchronisé sur `refonte-pre-lot-2`. P7 devient la recette complète avant toute décision de fusion vers `main`. Le détail vit dans [`P6_ETATS_TRANSVERSAUX.md`](../milestones/P6_ETATS_TRANSVERSAUX.md).
...
- À la demande de l'utilisateur, la règle historique imposant la lecture intégrale de ce journal et du document de phase avant chaque tâche est remplacée par une divulgation progressive : `AGENTS.md` court, `AGENT_CONTEXT.md` comme état courant, routage par tâche puis recherche ciblée avec `rg`.
...
**Clôture de la phase 11 et du lot 1 :** après les reprises successives du volet 4 et la validation ciblée de P11-F35, l'utilisateur confirme explicitement le 2 septembre 2026 la validation du jalon regroupé de la version Sites 31. P11-F18 à F35, les trois ensembles et AM1 sont donc acceptés dans leur état final ; CT1 demeure refusé. **Build de production, lint, `git diff --check` et 101/101 tests réussis.** Les cases de recette non relevées individuellement restent des limites de preuve et aucune conformité WCAG globale n'est revendiquée. Le backend, l'authentification, la persistance et les routes publiques dynamiques restent hors lot 1. La synchronisation GitHub directe et vérifiée est déclenchée par cette décision. Aucun lot 2 n'est ouvert implicitement ; son cadrage constitue la prochaine décision produit.

**Reprise sociale P11-F35 validée :** la recette de la version 30 invalide les traces de profil écrites séparément des critiques d'œuvre et la limitation des profils navigables à Lina. Le contrat devient transversal : toute trace publique provient de la même critique que la page d'œuvre et toute identité sociale affichée — avatar comme nom, critique comme réponse — ouvre son profil. Maël, Lina, Théo et Inès disposent désormais d'une destination et d'un suivi indépendants ; le backend social et les routes publiques dynamiques restent hors lot 1. Le 2 septembre 2026, l'utilisateur valide explicitement ce correctif sur la version 31 et comprend le report des cohérences dépendantes du backend. Cette validation ciblée ne vaut pas encore validation explicite du jalon regroupé.

**Précision de recette responsive :** le 1er septembre 2026, l'utilisateur confirme le défaut Safari de superposition et ajoute qu'une fenêtre desktop réduite à sa largeur minimale ne montre pas de face retournée et présente une composition étrange. Il autorise ensuite l'implémentation du périmètre diagnostiqué. Le correctif est candidat à l'évaluation, pas encore validé par recette : gestes photo continus et confinés, verso à une colonne, faces visuellement étanches, QRM1b/QRP1b conservés, tests ciblés ; aucune publication immédiate.

**Reprise après nouvelle capture mobile :** malgré le premier candidat, la capture montre encore le contenu du verso distribué comme une grille à deux colonnes. Le correctif ne repose plus uniquement sur la cascade extérieure : QR et identité appartiennent à un corps centré autonome entre en-tête et URL, tandis qu'une spécificité explicite verrouille la grille du verso. Dans le même retour, l'utilisateur demande la cohérence du suivi et de la recherche Découvrir ; les trois points d'entrée adoptent « Suivre/Suivi » et les couleurs du profil, et la croix WebKit est remplacée par l'action textuelle « Effacer ». Ces changements sont implémentés comme candidats à réévaluer, sans validation implicite.

**Décision de publication révisée :** le 27 août 2026, l'utilisateur demande d'abord de poursuivre plusieurs étapes sans déployer la version 25. Le 1er septembre, après constitution et reprise du jalon regroupé, il demande explicitement que les correctifs soient visibles sur le Site. La version 26 est donc déployée pour recette, puis remplacée par les versions 27 et 28 intégrant P11-F31 et sa reprise P11-F32 ; cette autorisation remplace l'interdiction de publication pour ces candidats de recette. La synchronisation GitHub des éléments validés demeure distincte et aucune validation utilisateur n'est déduite du déploiement.

Le deuxième ensemble est désormais validé explicitement : traitement du catalogue vide/incomplet sans remplacer une œuvre par une autre, états vides du Journal déjà validés, repli éditorial EH1 sans historique et couverture typographique après échec d'image. Cette validation autorise son implémentation, pas une publication. Les constats P11-F05 à F09 restent documentés dans le suivi de phase.

La question initiale de l'utilisateur sur les animations a été située dans le volet 3 de la phase 11, cohérence et accessibilité, avant la vérification finale. Cette discussion a abouti à la charte AM1 désormais validée ci-dessus. L'ancienne réserve « aucun nouvel effet autorisé » est levée pour ce seul périmètre ; elle ne devient pas une autorisation de refonte ni d'effets supplémentaires.

Résultat du deuxième ensemble : catalogue vide/incomplet sans substitution, écrits conservés, trois états du Journal raccordés, justification EH1 liée aux signaux disponibles et repli commun des couvertures après erreur. Build, lint et typage strict ciblé réussis ; **49 tests réussis**, dont 16 nouveaux rendus/gestions d'état. Aucun test navigateur ni déploiement. La suite porte sur les autres cas limites, puis le volet responsive, animations et accessibilité ; la phase 11 n'est pas clôturée.

Le troisième ensemble est désormais **validé explicitement et implémenté** : ordre fiable des imports photo, erreurs de recadrage récupérables, copie/partage sans chevauchement et écrits longs lisibles. Cette décision remplace le statut de proposition précédent. Les constats P11-F10 à F15 et le détail d'implémentation figurent dans le suivi de phase. PFP1, N1b, QRP1b, QRM1b, les assets et la limite de 3 000 caractères restent conservés. **Build, lint et typage strict ciblé réussis ; 70 tests réussis, dont 21 nouveaux.** Aucune recette navigateur ni publication.

L'utilisateur demande aussi une analyse de `https://github.com/emilkowalski/skills` pour le prochain volet. Les principaux guides de design engineering, recherche d'opportunités et audit ont été consultés à la révision `d23d7f88a2e21c9e4b1418c7abe420f5c1052ba7`, sans installation ni exécution de leurs instructions. Conclusion proposée : une grille de qualité utile pour le volet 3, centrée sur l'utilité/fréquence du mouvement, la cohérence et les alternatives accessibles. Présenter une courte charte de mouvement de Chapter et des comparaisons ciblées avant tout nouvel effet. Ne pas appliquer automatiquement la réduction d'échelle des boutons (contraire à S2), ni raccourcir le retournement QRM1b, actuellement fixé à environ 440 ms, au nom d'une règle générique. Les références, réserves et familles à examiner sont consignées au §11 du document de phase ; cette analyse n'est pas une nouvelle autorisation de développement.

- La phase 10 demeure clôturée sur la version 24. L'ouverture de la phase 11 ne constitue ni un nouveau jalon fonctionnel accepté, ni une demande de publication ou de synchronisation de changements non validés.
- Le travail conserve le périmètre d'interface sur données simulées, les compositions validées et les assets existants, notamment PDR1B, PFP1, N1b, QR1, HDE1, HV1, HMT1, les badges et le poinçon fourni.
- Ordre proposé, non encore validé : (1) superpositions, fermetures, focus et protection des saisies ; (2) états vides, erreurs et contenus extrêmes ; (3) responsive, lisibilité et accessibilité des contrôles ; (4) non-régression, bilan technique et recette finale exhaustive.
- Un examen statique initial relève l'absence de confinement et de restitution du focus dans plusieurs modales, l'absence de fermeture par Échap dans la confirmation de retrait de photo, et l'absence de transfert explicite du focus vers l'action sûre des protections de saisie. Ces constats ne sont pas présentés comme des tests navigateur exécutés.
...
- Le checkpoint 21 publie un correctif de recette : le composant QR exécuté dans le navigateur, incompatible avec la chaîne de production actuelle et responsable d'une page blanche à l'ouverture de « Mon profil », est remplacé par un SVG QR statique généré localement. L'URL encodée, la composition du verso et toutes les interactions QR1 restent inchangées ; le profil ne dépend plus d'un moteur QR au moment de son affichage. Le lint, la construction de production et neuf tests automatisés passent.
