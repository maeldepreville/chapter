...
test("the calendar presents a French month and staged day with labelled keyboard navigation", () => {
  ...
  assert.equal(nodes(tree, (node) => node.type === "select" && node.props?.["aria-label"] === "Mois").length, 1);
  assert.equal(nodes(tree, (node) => node.type === "input" && node.props?.["aria-label"] === "Année").length, 1);
  const day = days.find((node) => node.props["data-date"].endsWith("-15") && !node.props.className.includes("outside"));
  day.props.onClick();
  tree = render();
