...
test("P3 ships a material bookmark asset instead of a CSS banner", () => {
  const asset = fileURLToPath(new URL("../public/editorial/p3-reading-bookmark.webp", import.meta.url));
  const page = readFileSync(source("page.tsx"), "utf8");
  const styles = readFileSync(source("p3-personal.css"), "utf8");
  assert.ok(statSync(asset).size < 20_000);
  assert.match(page, /\/editorial\/p3-reading-bookmark\.webp/);
  assert.match(page, /current-reading-main--with-bookmark/);
  assert.match(page, /journal-row journal-reading-row/);
  assert.match(styles, /\.reading-bookmark-object img/);
  assert.match(styles, /\.journal-reading-row \{[^}]*grid-template-columns: minmax\(0, 1fr\) auto auto/);
  assert.doesNotMatch(styles, /\.reading-bookmark--compact[^}]+clip-path:\s*polygon/);
});
