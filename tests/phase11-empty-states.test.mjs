...
test("empty favorites and honors use distinct editorial assets", () => {
  ...
  assert.match(own, /p7-empty-favorites-v3\.webp/);
  assert.match(own, /p6-empty-profile\.webp/);
});
...
test("a private note has an explicit confirmed deletion with undo", () => withWindow(() => {
  ...
  ui.button("Supprimer la note").props.onClick();
  assert.doesNotMatch(textOf(ui.render()), /Une note à retirer/);
  const toast = nodes(ui.render(), (node) => node.type?.name === "ToastStack")[0].props.toasts.at(-1);
  assert.equal(toast.label, "Note privée supprimée");
  toast.onAction();
  assert.match(textOf(ui.render()), /Une note à retirer/);
}));
...
test("a public review can be removed while its private rating is preserved", () => withWindow(() => {
  ...
  const reviewRow = nodes(ui.render(), (node) => node.props?.className === "journal-row" && textOf(node).includes("Ma critique"))[0];
  ...
  const emptyReviewRow = nodes(ui.render(), (node) => node.props?.className === "journal-row" && textOf(node).includes("Ma critique"))[0];
