...
test("keeps the profile portrait coherent without an unnecessary local index", async () => {
  ...
  assert.match(stylesheet, /\.profile-library-portrait \.profile-section-heading--editable > div > \.eyebrow\s*\{[^}]*margin-bottom:\s*0\.4rem;/s);
  assert.match(stylesheet, /\.profile-honors-band \.profile-section-link > span:first-child\s*\{[^}]*gap:\s*0\.4rem;/s);
  assert.match(stylesheet, /\.profile-wide-content > \.profile-section\s*\{[^}]*padding-top:\s*clamp\(3\.5rem, 6vw, 5rem\);/s);
  ...
  assert.match(stylesheet, /@media \(max-width: 899px\)[\s\S]*\.destination-card\s*\{[^}]*height:\s*min\(88dvh, 46rem\);[^}]*max-height:\s*calc\(100% - 0\.5rem\);[^}]*border:\s*1px solid var\(--line\);[^}]*border-radius:\s*1\.1rem;/s);
